...
/**
 * Constants and miscellaneous functions useful to read Clic 3.0 files and import it into JClic
 * projects.
 *
...
public abstract class Clic3 {
  ...
  protected static final TripleColor DEFAULT_TRIPLE_COLOR =
      new TripleColor(Color.white, Color.black, Color.gray, false);

  // protected static final Font DEFAULT_FONT = new Font("Helvetica", 0, 12);

  // Extensions
  // better a hastable?
  public static final String extName[] = {
    "*.BMP", "*.GIF", "*.TXT", "*.PUZ", "*.SOP", "*.ASS", "*.CRW", "*.TXA", "*.PAC", "*.PCC",
    "*.P?C", "*.HLP", "*.MVB", "*.EXE", "*.MID", "*.WAV", "*.JPG", "*.PNG", BLANK
  };
  ...
  // public static final int MAX_MESS_LEN = 200;
  // public static final int MAX_FULL_FILENAME = 100, MAX_DESC = 200;
  ...
  /*
   * Constants used in Clic 3.0
   * not needed in JClic
   *
   * WM_USERIDS
   * WM_PLAYW=WM_USER, WM_HELPW, WM_CBNEXT, WM_CBPREV, WM_CBGETFOCUS,
   * WM_NOUINTENT};
   *
   * enum USERMESSAGES {PLAYW_RESIZE, PLAYW_ENDGAME, PLAYW_INTENT,
   * PLAYW_ENCERTS, PLAYW_WAITMEIN,
   * PLAYW_SETMESS, PLAYW_SETMESSPLAY, HELPW_ITEMSEL, HELPW_MOUSE};
   *
   * public static final int WDIGIT = 13, HDIGIT = 23;
   * public static final int MARGE = 6;
   * public static final int BOTOW = 63, BOTOW2 = 30, BOTOH = 39;
   *
   * public static final int DIGITSW = 39, DIGITSH = 23;
   *
   * public static final int CTRLH = 39;   // era 41
   * public static final int CTRLW = 450;
   *
   * public static final int CLW = (3*MARGE+BOTOW+CTRLW);
   * public static final int CLH = (6*BOTOH+7*MARGE);
   *
   * public static final int PLWW = CTRLW;
   * public static final int PLWH = (CLH-CTRLH-3*MARGE);
   *
   * public static final int LMAXRFTXT = 512;
   *
   * String resources (to be revised!)
   * public static final int IDS_ERRFMT = 1;
   *
   * Common variables
   *
   * public static int passos, passos0;
   * public static int gAjVis, encertsNow;
   * public static boolean quick, quick0, arrossega, arrossega0, sonor0,
   * autoCursor, cursorGran;
   * public static boolean doReportFile, drf0, exitDirect;
   * public static boolean helponhelp, menus;
   * public static boolean allowNewUsr, allowNewGrp, showUserReports;
   * public static int caretVis, caretX, caretY, caretW, caretH, autoCursorDelay;
   * public static final Font logFont1 = new Font("Arial", 0, 12);
   * public static int gruixfil;
   *
   * public static String clicPath = "";
   * public static String ediText = "";
   * public static String ediGraf = "";
   * public static String originalPath = "";
   * public static String playerId = "";
   * public static String iniFile = "";
   * public static String dbFile = "";
   * public static String originalDbFile = "";
   * public static String fileName0 = "";
   *
   * extern char cdUnit;
   * public static String lastExt = "   ";
   */
  ...
  public static int getExt(String fileName) {
    ...
    // lastExt=BLANK;
    if (fileName == null || fileName.length() == 0) return UNKNOWN_EXT;

    if ((begExt = fileName.indexOf(' ')) < 0) begExt = fileName.length() - 1;

    for (; begExt > 0; begExt--) if (fileName.charAt(begExt) == '.') break;

    if (begExt == 0 || fileName.length() - begExt < 4) return UNKNOWN_EXT;

    ext = "*" + fileName.substring(begExt, begExt + 4).toUpperCase();
    for (i = 0; i < UNKNOWN_EXT; i++) if (ext.equals(extName[i])) break;

    // lastExt=ext.substring(2,4);
    if (i == EXT_JPG || i == EXT_PNG) i = EXT_GIF;
  ...
  private static final String[] strArray = {
    "FFFFFF", "000000", "808080", "C0C0C0", "0000FF", "00FF00", "FF0000"
  };
  ...
  protected static Color strToColor(String st) {
    if (st == null) return Color.black;
    if (st.length() > 6) st = st.substring(st.length() - 6);
    for (int i = 0; i < strArray.length; i++) if (st.equals(strArray[i])) return colorArray[i];

    return new Color(correctColor(parseIntX(st, 16)));
  }

  public static boolean copyArray(
      String[] dest, int indest, String[] source, int insource, int e, boolean upper) {
    int i, d, s;

    for (i = 0, d = indest, s = insource; i < e; i++, d++, s++) {
      // if(dest[d]!=null) delete dest[d];
      // if(source[s]==null || (dest[d]=new String(source[s]))==null) break;
      if ((dest[d] = source[s]) == null) break;
      if (upper) dest[d] = dest[d].toUpperCase();
    }
    return (i == e);
  }
  ...
  public static void addActivityToBag(JClicProject project, String name) throws Exception {
    ...
    if (ab.getElementByName(normalizedName) != null) return;
  ...
  public static void addPacToSequence(JClicProject project, String name) throws Exception {
    ...
    if (project.activitySequence.getElementByTag(normalizedName, false) != null) return;
    ...
    for (int i = 0; i < c3p.nActs; i++) {
      ...
      if (i == 0) {
        ...
        // if(as.getCurrentActNum()<0)
        //    as.setCurrentAct(as.size());
      ...
      if (i == c3p.nActs - 1) {
        ...
        if (c3p.def == true && c3p.chDef != null && getExt(c3p.chDef) == EXT_PAC) {
          // s=fs.getRelativePathOf(relativePath+c3p.chDef)+FileSystem.getFileNameOf(c3p.chDef);
          asj.action = JumpInfo.JUMP;
          asj.sequence = fs.getCanonicalNameOf(c3p.chDef);
          if (c3p.sup) {
            // s=fs.getRelativePathOf(relativePath+c3p.chSup)+FileSystem.getFileNameOf(c3p.chSup);
            asj.setConditionalJump(
                new ConditionalJumpInfo(
                    JumpInfo.JUMP,
                    fs.getCanonicalNameOf(c3p.chSup),
                    c3p.supP,
                    c3p.supTFlag ? c3p.supT : -1),
                true);
          }
          if (c3p.inf) {
            // s=fs.getRelativePathOf(relativePath+c3p.chInf)+FileSystem.getFileNameOf(c3p.chInf);
            asj.setConditionalJump(
                new ConditionalJumpInfo(
                    JumpInfo.JUMP,
                    fs.getCanonicalNameOf(c3p.chInf),
                    c3p.infP,
                    c3p.infTFlag ? c3p.infT : -1),
                false);
          }
        }
        ase.fwdJump = asj;
  ...
  public static final String ansiChars =
      "\u20ac\u0081\u201a\u0192\u201e\u2026\u2020\u2021\u02c6\u2030\u0160\u2039\u0152"
          + "\u008d\u008e\u008f\u0090\u2018\u2019\u201c\u201d\u2022\u2013\u2014\u02dc\u2122"
          + "\u0161\u203a\u0153\u009d\u009e\u0178";
  public static final String convertFileChars =
      "e__f______s_c_z__________ts_o_zy_icl_y_s_ca___r_o_23__p__10_423_aaaaaaa"
  ...
  public static String bufferToString(byte[] buffer) {
    if (buffer == null || buffer.length == 0) return BLANK;
    ...
    for (int i = 0; i < buffer.length; i++) {
      ...
      if (v < 128 || v >= 160)
        ...
        result[i] = (char) v;
      // result[i]=(char)(v==45 ? 95 : v);
      else result[i] = ansiChars.charAt(v - 128);
    }
  ...
  public static String[] dataToArray(byte[] data) {
    ...
    l = data.length;
    for (i = 0, j = 0; i < l; ) {
      for (; j < l; j++) if (data[j] == 0x0D || data[j] == 0x0A) break;
      l2 = j - i;
      if (l2 > 0) {
        buffer = new byte[l2];
        for (k = 0; k < l2; k++) buffer[k] = data[i + k];
        v.add(bufferToString(buffer));
        // v.add(new String(buffer));

        // try{
        //    v.add(new String(buffer, "Cp1252"));
        // } catch(Exception ex){
        //    v.add(new String(buffer));
        // }
      } else v.add(BLANK);
      nLines++;
      if (j < l && data[++j] == 0x0A) j++;
      i = j;
    }
    ...
    for (char ch : name.toCharArray()) {
      if (validFileChars.indexOf(ch, strict ? 3 : 0) < 0) {
        if (ch >= 'A' && ch <= 'Z') ch = Character.toLowerCase(ch);
          ...
          if (k >= 0) ch = convertFileChars.charAt(k);
          else ch = '_';
        }
      }
      sb.append(ch);
  ...
  public static String pacNameToLowerCase(String s) {
    ...
    if (s != null) {
      ...
      if (l.endsWith(".pac") || l.endsWith(".pcc")) result = l;
    }
    return result;
  }
}
