...
/**
 * This class is a {@link javax.swing.JPanel} that contains a set of {@link
 * edu.xtec.jclic.boxes.ThumbsExplorerPanel.ThumbElement} objects. Elements are displayed into a
 * grid of rectangular cells of same size, distributed from left to right and top to bottom. It
 * provides methods to select a specific ThumbElement, and has a {@link
 * javax.swing.event.EventListenerList} used to notify observers about selection changes.
 *
...
public class ThumbsExplorerPanel extends JPanel {
  ...
  public void setSizes(
      int th_width, int th_height, int th_margin, int th_textHeight, int th_int_margin) {
  ...
  public ThumbElement addThumbElement(int p, Object userObject, ImageIcon img, String text) {
    ...
    if (p >= 0) elements.add(p, th);
    else elements.add(th);
    invalidate();
    return th;
  }
  ...
  public class ThumbElement {
        ...
        if (w > maxThumbSize.width || h > maxThumbSize.height) {
          ...
          image =
              new ImageIcon(
                  image
                      .getImage()
                      .getScaledInstance((int) (f * w), (int) (f * h), Image.SCALE_SMOOTH));
        }
      }
    }
    ...
    protected void paint(Graphics2D g2, Rectangle r) {
      ...
      if (text != null && text.length() > 0) {
        if (current == this) g2.setColor(LFUtil.getSysColor("activeCaptionText", Color.black));
        else g2.setColor(LFUtil.getColor("Table.foreground", Color.black));
