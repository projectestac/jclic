 ...
 * {@link edu.xtec.jclic.boxes.ThumbsExplorerPanel.ThumbElement} objects.
 * Elements are displayed into a grid of rectangular cells of same size,
 * distributed from left to right and top to bottom. It provides methods to
 * select a specific ThumbElement, and has a
 * {@link javax.swing.event.EventListenerList} used to notify observers about
 * selection changes.
 *
...
public class ThumbsExplorerPanel extends JPanel {
  ...
  public ThumbsExplorerPanel() {
    ...
    setSizes(DEFAULT_THUMB_WIDTH, DEFAULT_THUMB_HEIGHT, DEFAULT_THUMB_MARGIN, DEFAULT_TEXT_HEIGHT,
  ...
  public void doLayout() {
    ...
    /*
     * ELIMINATED DUE TO RECURSIVE CALLS IN 1.4 TEST IN 1.3.1
     *
     * int nRows=elements.size()/elementsPerRow+1; int h=nRows*boxSize.height;
     * if(getHeight()!=h) setBounds(getX(), getY(), w, h);
     */
  }
  ...
  public class ThumbElement {
    ...
    protected void sizeImage() {
      if (image != null) {
        ...
        if (w > maxThumbSize.width || h > maxThumbSize.height) {
          ...
          image = new ImageIcon(image.getImage().getScaledInstance((int) (f * w), (int) (f * h), Image.SCALE_SMOOTH));
        }
      }
    }
    ...
    protected void paint(Graphics2D g2, Rectangle r) {
      ...
      if (current == this) {
        Rectangle r2 = new Rectangle(r.x + th_margin / 2, r.y + th_margin / 2, r.width - th_margin,
            r.height - th_margin);
      ...
      if (image != null) {
        image.paintIcon(ThumbsExplorerPanel.this, g2, r.x + ((th_width + 2 * th_margin) - image.getIconWidth()) / 2,
      ...
      if (text != null && text.length() > 0) {
        ...
        layout.draw(g2, (float) r.x + th_margin + (th_width - (float) txRect.getWidth()) / 2,
