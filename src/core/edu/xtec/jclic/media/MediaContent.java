...
public class MediaContent extends Object implements Cloneable, Domable {
  ...
  public org.jdom.Element getJDomElement() {
    ...
    if (mediaType == PLAY_VIDEO) {
      ...
      if (catchMouseEvents) e.setAttribute(CATCH_MOUSE, JDomUtility.boolString(catchMouseEvents));
      if (absLocation != null) {
  ...
  public void setProperties(org.jdom.Element e, Object aux) throws Exception {
    ...
    if (mediaType == -1) throw new IllegalArgumentException("Unknown media type: " + mediaName);
        ...
        switch (type) {
          ...
          case RUN_CLIC_PACKAGE:
            map.put(
                s,
                p != null
                    ? Constants.EXTERNAL_OBJECT
                    : k > 0 ? Constants.ACTIVITY_OBJECT : Constants.SEQUENCE_OBJECT);
            if (p != null) map.put(p, Constants.EXTERNAL_OBJECT);
            break;
          case RUN_EXTERNAL:
  ...
  public Object clone() {
    ...
    try {
      ...
      if (absLocation != null) mc.absLocation = (Point) absLocation.clone();
    } catch (Exception ex) {
