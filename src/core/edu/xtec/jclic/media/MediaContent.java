...
public class MediaContent extends Object implements Cloneable, Domable {
  ...
  public static final int UNKNOWN = 0, PLAY_AUDIO = 1, PLAY_VIDEO = 2, PLAY_MIDI = 3, PLAY_CDAUDIO = 4,
      RECORD_AUDIO = 5, PLAY_RECORDED_AUDIO = 6, RUN_CLIC_ACTIVITY = 7, RUN_CLIC_PACKAGE = 8, RUN_EXTERNAL = 9,
      URL = 10, EXIT = 11, RETURN = 12, NUM_MEDIA_TYPES = 13;

  public static final String[] mediaName = { "UNKNOWN", "PLAY_AUDIO", "PLAY_VIDEO", "PLAY_MIDI", "PLAY_CDAUDIO",
      "RECORD_AUDIO", "PLAY_RECORDED_AUDIO", "RUN_CLIC_ACTIVITY", "RUN_CLIC_PACKAGE", "RUN_EXTERNAL", "URL", "EXIT",
      "RETURN" };
  ...
  protected static final String TYPE = "type", LEVEL = "level", FILE = "file", PARAMS = "params", FROM = "from",
      TO = "to", CDFROM = "cdFrom", CDTO = "cdTo", BUFFER = "buffer", LENGTH = "length", STRETCH = "stretch",
  ...
  public org.jdom.Element getJDomElement() {
    ...
    if (mediaFileName != null)
      e.setAttribute(FILE, mediaFileName);
    if (externalParam != null)
      e.setAttribute(PARAMS, externalParam);
    if (from != -1)
      e.setAttribute(FROM, Integer.toString(from));
    if (to != -1)
      e.setAttribute(TO, Integer.toString(to));
    if (cdFrom != null)
      e.setAttribute(CDFROM, cdFrom);
    if (cdTo != null)
      e.setAttribute(CDTO, cdTo);
    if (mediaType == RECORD_AUDIO || mediaType == PLAY_RECORDED_AUDIO) {
      e.setAttribute(BUFFER, Integer.toString(recBuffer));
      if (mediaType == RECORD_AUDIO)
        e.setAttribute(LENGTH, Integer.toString(length));
    }
    ...
    if (loop)
      e.setAttribute(LOOP, JDomUtility.boolString(loop));
    if (autoStart)
      e.setAttribute(AUTOSTART, JDomUtility.boolString(autoStart));
    return e;
  }
    ...
    if (e != null) {
      ...
      if (s != null && s.length() > 0) {
        ...
        case RUN_CLIC_PACKAGE:
          map.put(s,
              p != null ? Constants.EXTERNAL_OBJECT : k > 0 ? Constants.ACTIVITY_OBJECT : Constants.SEQUENCE_OBJECT);
          if (p != null)
  ...
  public boolean isEquivalent(MediaContent mc) {
    return mc != null && mediaType == mc.mediaType && StrUtils.compareStringsIgnoreCase(mediaFileName, mc.mediaFileName)
        && StrUtils.compareStringsIgnoreCase(externalParam, mc.externalParam) && from == mc.from && to == mc.to
        && StrUtils.compareStringsIgnoreCase(cdFrom, mc.cdFrom) && StrUtils.compareStringsIgnoreCase(cdTo, mc.cdTo)
        && recBuffer == mc.recBuffer && stretch == mc.stretch && free == mc.free
        && StrUtils.compareObjects(absLocation, mc.absLocation) && absLocationFrom == mc.absLocationFrom
        && loop == mc.loop && level == mc.level;
  }
  ...
  public String getDescription() {
    ...
    if (mediaFileName != null) {
      ...
      if (from >= 0)
        result.append(" ").append("from:").append(from);
      if (to >= 0)
        result.append(" ").append("to:").append(to);
