    ...
    class Panel extends TextActivityBase.Panel {
        ...
        class IdentifyPane extends TextActivityPane{
            ...
            protected void select(int p){
                ...
                playDoc.setCharacterAttributes(p, l, styleContext.getStyle(marked ? StyleContext.DEFAULT_STYLE : TextActivityDocument.TARGET), true);
                ...
                if(!hasCheckButton){
                    ...
                    try{
                        tabp.ps.reportNewAction(Identify.this, ACTION_SELECT, playDoc.getText(p, l), (marked ? "true":"false"), ok, score);
                    } catch(Exception ex){
                        // should never done
                    }
                    if(finishedOk){
        ...
        protected void doCheck(boolean fromButton){
            ...
            for(i=0; i<tmb.size(); i++){
                ...
                playDoc.applyStyleToTarget(tm, marks[tm.begOffset] ? TextActivityDocument.TARGET : StyleContext.DEFAULT_STYLE, false, true);
            }
            boolean finishedOk=checkScore();
