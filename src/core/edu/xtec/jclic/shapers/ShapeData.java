...
public class ShapeData extends Object implements Cloneable {
  ...
  public org.jdom.Element getJDomElement(double scaleW, double scaleH) {
    ...
    if (comment != null && comment.length() > 0) e.setAttribute(COMMENT, comment);
  ...
  public static ShapeData getShapeData(org.jdom.Element e, double scaleW, double scaleH)
      throws Exception {
      ...
      for (int k = 0; k < NUM_PRIMITIVES; k++) {
        if (s.equals(PRIMITIVES[k])) {
          ...
          switch (k) {
            ...
            case ROUND_RECT:
              sh =
                  new RoundRectangle2D.Double(data[0], data[1], data[2], data[3], data[4], data[5]);
              break;
            case ARC:
              sh =
                  new Arc2D.Double(
          ...
          if (sh != null) return getShapeData(sh, sd.comment);
        }
      }
      if (s.equals(MOVE_TO)) {
  ...
  private void writeToSb(
      StringBuilder sb, String type, double[] values, int j, int k, double scaleW, double scaleH) {
    sb.append(type);
    if (k > 0) {
      if (DF == null) {
  ...
  private void addData(double[] data) {
    ...
    for (double d : data) points[pointsIndex++] = d;
  }

  private static double[] decodeData(StringTokenizer st, int count, double scaleW, double scaleH)
      throws Exception {
  ...
  public Shape getShape(double dx, double dy, double scaleX, double scaleY) {
    ...
    for (int i = 0; i < descriptorsIndex; i++) {
      switch (descriptors[i]) {
        ...
        case PathIterator.SEG_QUADTO:
          gp.quadTo(
              (float) (dx + scaleX * points[j++]),
              (float) (dy + scaleY * points[j++]),
              (float) (dx + scaleX * points[j++]),
              (float) (dy + scaleY * points[j++]));
          break;
        case PathIterator.SEG_CUBICTO:
  ...
  public Object clone() throws CloneNotSupportedException {
    ...
    if (points != null) sd.points = (double[]) points.clone();
    if (descriptors != null) sd.descriptors = (int[]) descriptors.clone();
    if (primitivePoints != null) sd.primitivePoints = (double[]) primitivePoints.clone();
    return sd;
  }
}
