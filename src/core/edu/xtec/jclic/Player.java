...
/**
 * <CODE>Player</CODE> is one of the the main classes of the JClic system. It implements the {@link
 * edu.xtec.jclic.PlayStation} interface, so it can read and play JClic projects from files or
 * streams. In order to allow activities to run, <CODE>Player</CODE> provides them of all the
 * necessary resources: media bags (to load and realize images and other media contents), sequence
 * control, report system management, user interface (loading and management of skins), display of
 * system messages, etc. Player is also a {@link edu.xtec.jclic.RunnableComponent}, so it can be
...
public class Player extends JPanel
    implements Constants, RunnableComponent, PlayStation, ActionListener {
  ...
  /**
   * List of the names of the internal resources corresponding to the default event sounds used by
   * JClic.
   */
  ...
  /**
   * List of the names of the internal resources corresponding to the default icons associated to
   * the basic actions defined in {@link edu.xtec.jclic.Constants}.
   */
  ...
  /** The {@link edu.xtec.jclic.project.JClicProject} currently hosted by the {@code Player}. */
  protected JClicProject project;

  /**
   * The UI element (<CODE>Panel</CODE>)of the <CODE>Activity</CODE> currently running in the <CODE>
   * Player</CODE>.
   */
  protected Activity.Panel actPanel;

  /**
   * This object manages a list with the names of all the activities currently played by the user in
  ...
  /**
   * Default skin of this <CODE>Player</CODE>. Users can override the <CODE>DEFAULT_SKIN</CODE>
   * setting and choose another Skin to be used by default in JClic.
   */
  ...
  /**
   * Main <CODE>Timer</CODE>, used to feed the time conter. This timer generates an <CODE>
   * ActionPerformed</CODE> event every second.
   */
  protected Timer timer;

  /**
   * This flag indicates if the <CODE>Player</CODE> must play the sounds (including system sounds)
   * and other media contents of the activities.
   */
  ...
  /**
   * This flag indicates if the navigation buttons (<I>go to next activity</I> and <I>go back</I>)
   * are enabled o disabled.
   */
  protected boolean navButtonsDisabled = false;

  /**
   * When this flag is <CODE>true</CODE>, the navigation buttons are always enabled, despite of the
   * indications made by the activities or the sequence control system. Used only for debugging
   * projects with complicated sequence chaining.
   */
  ...
  /**
   * Creates a new <CODE>Player</CODE> object that will be initially loaded with a specific <CODE>
   * JClicProject</CODE>.
   *
   * @param options Options object to be used in the initialization process
  ...
  /**
   * Main initialization process, called once by constructors. Subclasses of <CODE>Player</CODE>
   * should override this method to initialize additional members.
   */
  protected void init() {
    ...
    for (int i = 0; i < NUM_COUNTERS; i++) counterVal[i] = 0;
    setMessages();
    buildActions();
    setActionsText();
    ...
    if (skin != null && skin.hasMemMonitor()) skin.setMem(Runtime.getRuntime().freeMemory());
    setWaitCursor(false);
    setWindowTitle();
  ...
  /**
   * Starts the player, loading a specific project if specified. This method is defined in the
   * {@link RunnableComponent} interface.
   *
   * @param fullPath Full path to the JClic project file to be loaded. Can be <I>null</I>.
   * @param sequence Optional parameter, used only when <CODE>fullPath</CODE> is not <I>null</I>. It
   *     indicates the sequence where the to start. It's also possible to indicate a string
   *     representation of a number "N". In this case, the player will start with the activity
   *     indicated by the Nth element of the main sequence of the project.
   * @return <CODE>true</CODE> if the player starts successfully. <CODE>false</CODE> otherwise.
   */
  public boolean start(String fullPath, String sequence) {
    initReporter();
    if (fullPath != null) return load(fullPath, sequence);
    else return false;
  }

  /**
   * This method is called when the container gains the focus for the first time or when losts it.
   * Not used in <CODE>Player</CODE>.
   */
  ...
  public void end() {
    ...
    if (activeMediaBag != null) activeMediaBag.removeAll();
    if (reporter != null) {
      reporter.end();
      reporter = null;
    }
    ...
    try {
      ...
      cursors[HAND_CURSOR] =
          tk.createCustomCursor(
              ResourceManager.getImageIcon("cursors/hand.gif").getImage(), new Point(8, 0), "hand");

      cursors[OK_CURSOR] =
          tk.createCustomCursor(
              ResourceManager.getImageIcon("cursors/ok.gif").getImage(), new Point(0, 0), "ok");
  ...
  protected void initReporter() {
    ...
    String reporterClassName =
        StrUtils.secureString(options.getString(REPORTER_CLASS), DEFAULT_REPORTER);
    try {
      reporter =
          Reporter.getReporter(
              reporterClassName, options.getString(REPORTER_PARAMS), this, messages);
    } catch (Exception ex) {
      reporter = null;
  ...
  /**
   * Creates and initializes the members {@link timer}, {@link delayedTimer} and {@link
   * delayedAction}
   */
  ...
  public void closeHelpWindow() {
    if (skin != null) {
      if (skin.currentHelpWindow != null) skin.currentHelpWindow.setVisible(false);
      if (skin.currentAboutWindow != null) skin.currentAboutWindow.setVisible(false);
    }
  }
  ...
  public void setSkin(Skin newSkin) {
    if (newSkin == null) newSkin = defaultSkin;
    ...
    if (newSkin != null && !newSkin.equals(skin)) {
      ...
      if (top != null) {
        ...
        while (top != null && rpc == null) {
          if (top instanceof RootPaneContainer) rpc = (RootPaneContainer) top;
      ...
      if (currentSkinSettings != null && skin != null) skin.setCurrentSettings(currentSkinSettings);
    }
  }
  ...
  public void load(
      final String sFullPath,
      final String sSequence,
      final String sActivity,
    ...
    worker =
        new edu.xtec.util.SwingWorker() {
          ...
          public Object construct() {
            ...
            if (skin != null) skin.startAnimation();
            ...
            try {
              ...
              if (fullPath != null) {
                setSystemMessage(
                    messages.get("msg_loading_project"), FileSystem.getFileNameOf(fullPath));
                ...
                if (projectName.endsWith(".jclic")) {
                  ...
                  JClicProject prj =
                      JClicProject.getJClicProject(
                          doc.getRootElement(), thisPlayer, fileSystem, fullPath);
                  if (json != null) prj.readJSON(json, false);
                  setProject(prj);
                  if (reporter != null) reporter.newSession(project, thisPlayer, messages);
                } else {
              ...
              if (sequence != null) {
                ...
                setSystemMessage(
                    messages.get("msg_loading_project"), FileSystem.getFileNameOf(seqName));

                navButtonsDisabled = false;
                ActivitySequenceElement ase =
                    project.activitySequence.getElementByTag(seqName, true);
                ...
                if (ase == null) {
                  ...
                  if (isPcc || isPac) {
                    if (isPcc) {
                      ...
                      } else setProject(new JClicProject(thisPlayer, fileSystem, path));
                      firstPac = true;
                ...
                if (ase != null) {
                  if (reporter != null) reporter.newSequence(ase);
              ...
              if (act != null) {
                ...
                if (abe != null) project.activitySequence.checkCurrentActivity(abe.getName());
          ...
          public void finished() {
            ...
            if (actp != null && worker != null) {
              ...
              // set skin
              if (skin != null) skin.resetAllCounters(false);

              if (actp.skin != null) setSkin(actp.skin);
              else if (project.skin != null) setSkin(project.skin);
              else setSkin(defaultSkin);

              if (skin != null) {
                boolean hasReturn = (history.storedElementsCount() > 0);
                int navBtnFlag =
                    navButtonsAlways
                        ? ActivitySequenceElement.NAV_BOTH
                        : navButtonsDisabled
                            ? ActivitySequenceElement.NAV_NONE
                            : project.activitySequence.getNavButtonsFlag();

                if (actions != null) {
                  actions[ACTION_NEXT].setEnabled(
                      (navBtnFlag & ActivitySequenceElement.NAV_FWD) != 0
                          && project.activitySequence.hasNextAct(hasReturn));
                  actions[ACTION_PREV].setEnabled(
                      (navBtnFlag & ActivitySequenceElement.NAV_BACK) != 0
    ...
    forceFinishActivity();
    if (skin != null) skin.setEnabled(false);
    setEnabled(false);
    worker.start();
  }
    ...
    if (timer != null) {
      ...
      if (actPanel != null) {
        ...
        if (Utils.lowMemoryCondition()) {
          if (trace) System.out.println(">>> LOW MEMORY! cleaning...");
  ...
  public void initActivity() {
    ...
    try {
      if (actPanel != null) {
        ...
        timer.start();
        if (!actPanel.getActivity().mustPauseSequence()) startAutoPassTimer();
        if (getFressa() != null) getFressa().initActivity(actPanel);
        setSystemMessage(messages.get("msg_activity_running"), null);
      }
      if (skin != null) skin.setMem(Runtime.getRuntime().freeMemory());
    } catch (Exception ex) {
  ...
  public void doLayout() {
    if (trace) System.out.println(">>> layout!");
    if (actPanel != null) {
      BoxBase.resetAllFonts();
      Rectangle bounds = getBounds();
      Rectangle proposedRect =
          new Rectangle(
              AC_MARGIN, AC_MARGIN, bounds.width - 2 * AC_MARGIN, bounds.height - 2 * AC_MARGIN);
  ...
  public void paintComponent(Graphics g) {
    ...
    if (actPanel == null
        || actPanel.getActivity().bgGradient == null
        || actPanel.getActivity().bgGradient.hasTransparency()) super.paintComponent(g);

    if (actPanel != null
        && (actPanel.getActivity().bgGradient != null || actPanel.bgImage != null)) {
      ...
      if (actPanel.bgImage != null) {
        Rectangle r =
            new Rectangle(0, 0, actPanel.bgImage.getWidth(this), actPanel.bgImage.getHeight(this));
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (timer != null && e.getSource().equals(timer)) {
      ...
      if (actPanel != null
          && actPanel.getActivity().maxTime > 0
          && actPanel.isPlaying()
    ...
    if (ac == null && (ac = e.getActionCommand()) == null) return;
    delayedAction = null;
  ...
  protected void buildActions() {
    ...
    actions[ACTION_RESET] =
        new AbstractAction() {
          public void actionPerformed(ActionEvent ev) {
            if (actPanel != null && actPanel.getActivity().canReinit()) initActivity();
          }
        };
    ...
    actions[ACTION_HLP] =
        new AbstractAction() {
          public void actionPerformed(ActionEvent ev) {
            if (actPanel != null) actPanel.showHelp();
          }
        };
    ...
    actions[ACTION_AUDIO].putValue(
        AbstractAction.DEFAULT, audioEnabled ? Boolean.TRUE : Boolean.FALSE);
      ...
      for (int i = 0; i < actions.length; i++) {
        if (actions[i] != null) {
          ...
          if (s != null && s.length() == 2) {
            ...
            if (kk >= 0)
              actions[i].putValue(
                  Action.ACCELERATOR_KEY, KeyStroke.getKeyStroke(kk, KeyEvent.CTRL_MASK));
          }
      ...
      try {
        ...
        if (project != null) {
          ...
          aw.buildStandardTab(
              ...
              "about_window_lb_project",
              "icons/info_small.gif");
        }
        if (reporter != null) {
          aw.buildStandardTab(
              reporter.toHtmlString(messages),
              "about_window_reportInfo",
              "about_window_lb_report",
              "icons/report_small.gif");
          if (selectReportPane) aw.getTabbedPane().setSelectedIndex(3);
        }
          ...
          public void run() {
            ...
            switch (mediaContent.mediaType) {
              ...
              case MediaContent.EXIT:
                ...
                if (exitUrl != null) exitUrl = project.getFileSystem().getUrl(exitUrl);
                ...
                if (audioEnabled) {
                  ActiveMediaPlayer amp =
                      activeMediaBag.getActiveMediaPlayer(
                          mediaContent, project.mediaBag, Player.this);
                  if (amp != null) {
  ...
  public void activityFinished(boolean completedOk) {
    ...
    if (getFressa() != null) getFressa().activityFinished();
  ...
  public void setMsg(ActiveBoxContent abc) {
    ActiveBox ab = null;
    if (skin != null) ab = skin.getMsgBox();
    if (ab != null) {
      ab.clear();
  ...
  public void incCounterValue(int counterId) {
    ...
    Counter c = null;
    if (skin != null && (c = skin.getCounter(counterId)) != null) c.setValue(counterVal[counterId]);
    if (counterId == ACTIONS_COUNTER
        && actPanel != null
        && actPanel.getActivity().maxActions > 0
        && actPanel.isPlaying()
        && counterVal[ACTIONS_COUNTER] >= actPanel.getActivity().maxActions) {
      // 14-Mai-2010
      // Correction of bug 1249: Incorrect reporting of activity results when maximum number of
      // attempts achieved
  ...
  public void setCountDown(int counterId, int maxValue) {
    Counter c = null;
    if (skin != null && (c = skin.getCounter(counterId)) != null) c.setCountDown(maxValue);
  }

  public void setCounterValue(int counterId, int newValue) {
    counterVal[counterId] = newValue;
    Counter c = null;
    if (skin != null && (c = skin.getCounter(counterId)) != null) c.setValue(newValue);
  }
  ...
  public void setSystemMessage(String msg1, String msg2) {
    ...
    if (skin != null) skin.setSystemMessage(msg1, msg2);
  ...
  public Cursor getCustomCursor(int type) {
    if (type >= 0 && type < cursors.length) return cursors[type];
    else return null;
  }
  ...
  public void reportNewActivity(Activity act, int currentScore) {
    ...
    if (reporter != null) {
      ...
      if (act.includeInReports) reporter.newActivity(act);
    }
    setCounterValue(ACTIONS_COUNTER, 0);
    setCounterValue(SCORE_COUNTER, 0);
  }

  public void reportNewAction(
      Activity act, String type, String source, String dest, boolean ok, int currentScore) {
  ...
  public void requestFocus() {
    if (actPanel != null) actPanel.requestFocus();
  }
  ...
  public void setWindowTitle(String docTitle) {
    ...
    if (w != null) {
      ...
      sb.append(appName);
      if (w instanceof Frame) ((Frame) w).setTitle(sb.substring(0));
      else if (w instanceof Dialog) ((Dialog) w).setTitle(sb.substring(0));
    }
  }
  ...
  public void setWindowTitle() {
    ...
    String actName =
        actPanel == null ? null : StrUtils.nullableString(actPanel.getActivity().getPublicName());
    if (actName != null) {
      sb.append(actName);
      if (prjName != null) sb.append(" [");
    }
    if (prjName != null) {
      sb.append(prjName);
      if (actName != null) sb.append("]");
    }
