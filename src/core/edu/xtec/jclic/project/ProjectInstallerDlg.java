 ...
 * @version 13.08.08
 */
public class ProjectInstallerDlg extends ExtendedJDialog
    implements ProgressInputStream.ProgressInputStreamListener, ListSelectionListener {
  ...
  private static final int STAGE_INFO = 0,
      STAGE_SELECT_LIB = 1,
      STAGE_COPY_FILES = 2,
      STAGE_SELECT_FOLDER = 3,
      STAGE_END = 4,
      NUM_STAGES = 5;
  private static final String[] STAGE_TITLES =
      new String[] {
        "install_title", "install_step_1", "install_step_2", "install_step_3", "install_step_4",
      };
  private static final String LOGO = "icons/install.png";
  /** Creates new form ProjectInstallerDlg */
  private ProjectInstallerDlg(Component parent, LibraryManager lm, ProjectInstaller pi)
      throws Exception {
    ...
    this.pi = pi;
    if (pi.from == null) throw new Exception("Path to installer data store not specified!");
  ...
  public static ProjectInstallerDlg getProjectInstallerDlg(
      Component parent, LibraryManager lm, String installerScriptFile) throws Exception {
    return new ProjectInstallerDlg(
        parent, lm, ProjectInstaller.getProjectInstaller(installerScriptFile));
  }
  ...
  private void initComponents() { // GEN-BEGIN:initComponents
    ...
    logoPanel =
        new javax.swing.JPanel() {
          public void paintComponent(Graphics g) {
            ...
            if (logoGp != null) logoGp.paint((Graphics2D) g, r);
            if (logo != null) {
    ...
    libFolderPanel.setBorder(
        new javax.swing.border.TitledBorder(msg.get("install_library_folders")));
  ...
  private void goToStage(int stage) {
    if (stage >= 0 && stage <= NUM_STAGES) {
      ...
      StringBuilder nbs =
          new StringBuilder("wizard_").append(currentStage < NUM_STAGES - 1 ? "next" : "finish");
  ...
  private void initCopyStage() {
    ...
    if (pl == null || !pl.getFullPath().equals(lme.path)) {
      try {
        ...
        if (etp != null) libFolderPanel.remove(etp);
        etp =
            new EditorTreePanel(pl.getEditor(null), options, true, Menu.class) {
              @Override
  ...
  private void initEndStage() {
    try {
      ...
      for (int i = 0; i < pi.iconItems.size() && !cancel; i++) {
        ...
        if (ii.icon != null) {
          if (!ii.icon.startsWith("@")) me.icon = pi.baseFolder + File.separator + ii.icon;
          else me.icon = ii.icon;
        } else me.icon = MenuElement.DEFAULT_ICON;
  ...
  private void prevBtnActionPerformed(
      java.awt.event.ActionEvent evt) { // GEN-FIRST:event_prevBtnActionPerformed
        ...
        new edu.xtec.util.SwingWorker() {
          ...
          public Object construct() {
            ...
            if (srcFolder.equalsIgnoreCase(destFolder)) {
              ...
              try {
                if (msg.confirmOverwriteFile(cmp, installDir, "yn") != Messages.YES) return PAUSED;
                ...
                for (int i = 0; i < pi.files.size() && !cancel; i++) {
                  ...
                  if (is instanceof ProgressInputStream) pis = (ProgressInputStream) is;
                  else {
                  ...
                  if (cancel || ex != null) {
                    try {
                      ...
                      msg.showErrorWarning(
                          cmp, "FILE_ERR_RESTORING", fb.getFile().getAbsolutePath(), e, null);
                    }
                  } else {
                    try {
                      fb.cleanup();
                      ...
                      msg.showErrorWarning(
                          cmp, "FILE_ERR_DELETING", fb.getBackup().getAbsolutePath(), e, null);
                    }
                  }
                }
              }
