...
public class CopyActivityAttributesPanel extends javax.swing.JPanel {
  ...
  private void initComponents() {
    ...
    actPanel.setBorder(
        javax.swing.BorderFactory.createTitledBorder(options.getMsg("edit_act_copy_dest")));
  ...
  private void selectNoneBtnActionPerformed(
      java.awt.event.ActionEvent evt) { // GEN-FIRST:event_selectNoneBtnActionPerformed

    actList.clearSelection();
  } // GEN-LAST:event_selectNoneBtnActionPerformed

  private void selectAllBtnActionPerformed(
      java.awt.event.ActionEvent evt) { // GEN-FIRST:event_selectAllBtnActionPerformed
  ...
  private TreeModel getTreeModel() {
    DefaultMutableTreeNode root =
        new DefaultMutableTreeNode(options.getMsg("edit_act_copy_attributes"));

    // Main Window
    DefaultMutableTreeNode node =
        new DefaultMutableTreeNode(options.getMsg("edit_act_copy_style_main"));
    ...
    node.add(
        activityBgColor = new CheckBoxTreeObject(options.getMsg("edit_act_copy_bgColor"), true));
    node.add(
        activityBgGradient =
            new CheckBoxTreeObject(options.getMsg("edit_act_copy_gradient"), true));
    node.add(
        transparentBg =
            new CheckBoxTreeObject(options.getMsg("edit_act_copy_transparentBg"), true));
    ...
    node.add(
        repInclude = new CheckBoxTreeObject(options.getMsg("edit_act_copy_rep_include"), false));
    node.add(
        repActions = new CheckBoxTreeObject(options.getMsg("edit_act_copy_rep_actions"), false));
    root.add(node);
    ...
    node.add(
        targetColour =
            new CheckBoxTreeObject(options.getMsg("edit_act_copy_textTargetColour"), true));
    node.add(
        errorColour =
            new CheckBoxTreeObject(options.getMsg("edit_act_copy_textErrorColour"), true));
  ...
  private void copyAttributes(Activity srcAct, Activity targetAct) {
    ...
    // Main window
    if (bgColor.isSelected()) targetAct.bgColor = srcAct.bgColor;
    if (bgGradient.isSelected()) targetAct.bgGradient = srcAct.bgGradient;
    if (bgImage.isSelected()) {
      targetAct.bgImageFile = srcAct.bgImageFile;
      targetAct.tiledBgImg = srcAct.tiledBgImg;
    }
    if (margin.isSelected()) targetAct.margin = srcAct.margin;

    // Play window
    if (activityBgColor.isSelected()) targetAct.activityBgColor = srcAct.activityBgColor;
    if (activityBgGradient.isSelected()) targetAct.activityBgGradient = srcAct.activityBgGradient;
    if (transparentBg.isSelected()) targetAct.transparentBg = srcAct.transparentBg;
    if (border.isSelected()) targetAct.border = srcAct.border;
    ...
    // messages
    if (msgPrev.isSelected()
        && srcAct.messages[Activity.PREVIOUS] != null
    ...
    if (msgIni.isSelected()
        && srcAct.messages[Activity.MAIN] != null
        && targetAct.messages[Activity.MAIN] != null)
      srcAct.messages[Activity.MAIN].copyStyleTo(targetAct.messages[Activity.MAIN]);
    if (msgEnd.isSelected()
        && srcAct.messages[Activity.END] != null
        && targetAct.messages[Activity.END] != null)
      srcAct.messages[Activity.END].copyStyleTo(targetAct.messages[Activity.END]);
    if (msgErr.isSelected()
        && srcAct.messages[Activity.END_ERROR] != null
    ...
    // reports
    if (repInclude.isSelected()) targetAct.includeInReports = srcAct.includeInReports;
    if (repActions.isSelected()) targetAct.reportActions = srcAct.reportActions;

    // interface
    if (skin.isSelected()) targetAct.skinFileName = srcAct.skinFileName;
    if (evSounds.isSelected()) targetAct.eventSounds = srcAct.eventSounds;

    // counters
    if (cTime.isSelected()) targetAct.bTimeCounter = srcAct.bTimeCounter;
    if (maxTime.isSelected()) {
      targetAct.maxTime = srcAct.maxTime;
      targetAct.countDownTime = srcAct.countDownTime;
    }
    if (cTries.isSelected()) targetAct.bActionsCounter = srcAct.bActionsCounter;
    if (maxTries.isSelected()) {
      targetAct.maxActions = srcAct.maxActions;
      targetAct.countDownActions = srcAct.countDownActions;
    }
    if (cScore.isSelected()) targetAct.bScoreCounter = srcAct.bScoreCounter;
    if (scramble.isSelected()) targetAct.shuffles = srcAct.shuffles;
    ...
    if (gridA.isSelected()
        ...
        && srcAct.abc[0] != null
        && targetAct.abc != null
        && targetAct.abc.length > 0
        && targetAct.abc[0] != null) srcAct.abc[0].copyStyleTo(targetAct.abc[0]);
    if (gridB.isSelected()
        && srcAct.abc != null
        && srcAct.abc.length > 1
        && srcAct.abc[1] != null
        && targetAct.abc != null
        && targetAct.abc.length > 1
        && targetAct.abc[1] != null) srcAct.abc[1].copyStyleTo(targetAct.abc[1]);
    ...
    if (srcAct instanceof TextActivityBase && targetAct instanceof TextActivityBase) {
      ...
      if (textFontFace.isSelected()
          || textFontSize.isSelected()
          || textStyle.isSelected()
          || textColour.isSelected()
          || targetColour.isSelected()
          || errorColour.isSelected()
          || tabSpace.isSelected()) {
        StyleContext sc =
            TextActivityDocument.copyStylesFrom(
                txSrcAct.getStyleContext(), txTargetAct.getStyleContext(),
                textFontFace.isSelected(), textFontSize.isSelected(),
                textStyle.isSelected(), textColour.isSelected(),
