 ...
 * @version 13.09.16
 */
public class TextTargetEditorPanel extends javax.swing.JPanel
  ...
  protected static final int[] popValues = {
    TextTarget.NO_INFO, TextTarget.INFO_ALWAYS, TextTarget.INFO_ON_ERROR, TextTarget.INFO_ON_DEMAND
  };
  protected static final String[] popKeys = {"no", "always", "onError", "onDemand"};
  protected String[] popItems;

  protected static final Map<Options, TextTargetEditorPanel> panels =
      new HashMap<Options, TextTargetEditorPanel>(1);
  ...
  public TextTargetEditorPanel(Options options, MediaBagEditor mbe) {
    ...
    popupPreview.setBounds(
        0, 0, popupPreview.getPreferredSize().width, popupPreview.getPreferredSize().height);
  ...
  private void initComponents() {
    ...
    popupPanel.setBorder(
        javax.swing.BorderFactory.createTitledBorder(options.getMsg("edit_text_act_popup")));
  ...
  public void setTextTarget(edu.xtec.jclic.activities.text.TextTarget textTarget) {
    ...
    iniText.setText(
        textTarget != null ? (textTarget.iniText != null ? textTarget.iniText : "") : "");
    listEditor.setTextList(textTarget != null ? textTarget.options : null);

    int v = 0;
    if (textTarget != null) v = Math.max(Arrays.binarySearch(popValues, textTarget.infoMode), 0);
  ...
  public void checkEnabled() {
    ...
    else ((CardLayout) cardPanel.getLayout()).first(cardPanel);
  }

  public void collectData() {
    if (textTarget != null /*&& isModified()*/) {
      String[] answers = answerListEditor.getTextList();
      if (answers.length > 0) textTarget.answer = answers;
    ...
    if (jc == jpab) {
      ActiveBoxContent abc =
          ActiveBoxContentEditor.getActiveBoxContent(
              jpab.ab.getContent(), jpab, options, mbe, jpab.ab);
      if (abc != null) {
        Dimension d = abc.getDimension();
        jpab.setActiveBoxContent(abc);
        if (d != null) jpab.setBounds(0, 0, d.width, d.height);
        // setModified(true);
      }
    }
  }
  ...
  public static boolean editTextTarget(
      TextTarget tt, Options options, MediaBagEditor mbe, Component parent) {
