...
public class ActivityEditorFramePanel extends ActivityEditorInternalPanel {
  ...
  private void initComponents() { // GEN-BEGIN:initComponents
    ...
    previewPanel0.setBorder(
        new javax.swing.border.TitledBorder(options.getMsg("edit_act_preview")));
    ...
    mainWindowPanel.setBorder(
        new javax.swing.border.TitledBorder(options.getMsg("edit_act_mainWindow")));
    ...
    gameWindowPanel.setBorder(
        new javax.swing.border.TitledBorder(options.getMsg("edit_act_gameWindow")));
  ...
  public void fillData() {
    ...
    ((ColorButton) actBgColorBtn)
        .setColor(act == null ? Activity.DEFAULT_BG_COLOR : act.activityBgColor);
  ...
  class PreviewPanel extends JPanel {
    ...
    class InnerPreviewPane extends JPanel {
      ...
      public void fitTo(Rectangle proposedRect, Rectangle maxRect) {
        ...
        int w =
            Math.max(Activity.MINIMUM_WIDTH, Math.min(proposedRect.width, (int) (scale * d.width)));
        int h =
            Math.max(
                Activity.MINIMUM_HEIGHT, Math.min(proposedRect.height, (int) (scale * d.height)));
        ...
        int xp =
            (b ? 0 : bgImageOrigin.x)
                + Math.max(
                    0,
                    b
                        ? proposedRect.x + (proposedRect.width - w) / 2
                        : (int) (scale * xInt.getValue()));
        int yp =
            (b ? 0 : bgImageOrigin.y)
                + Math.max(
                    0,
                    b
                        ? proposedRect.y + (proposedRect.height - h) / 2
                        : (int) (scale * yInt.getValue()));

        if (xp > maxRect.width - Activity.MINIMUM_WIDTH)
          xp = maxRect.width - Activity.MINIMUM_WIDTH;
        if (xp + w > maxRect.width) w = maxRect.width - xp;

        if (yp > maxRect.height - Activity.MINIMUM_HEIGHT)
          yp = maxRect.height - Activity.MINIMUM_HEIGHT;
        if (yp + h > maxRect.height) h = maxRect.height - yp;
    ...
    public void doLayout() {
      ...
      Rectangle proposedRect =
          new Rectangle(margin, margin, bounds.width - 2 * margin, bounds.height - 2 * margin);
    ...
    public void paintComponent(Graphics g) {
      ...
      if (bgGradient == null || bgGradient.hasTransparency()) super.paintComponent(g);

      if (bgGradient != null || bgImage != null) {
        RenderingHints rh = g2.getRenderingHints();
        g2.setRenderingHints(edu.xtec.jclic.Constants.DEFAULT_RENDERING_HINTS);

        if (bgGradient != null) bgGradient.paint(g2, rBounds);
        ...
        if (bgImage != null) {
          ...
          if (!tiledBgImg) {
            ...
            if (r.intersects(gBounds)) {
              if (scale == 1.0) g2.drawImage(bgImage, bgImageOrigin.x, bgImageOrigin.y, this);
              else {
