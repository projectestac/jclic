...
public class MediaContentEditor extends edu.xtec.util.CtrlPanel {
  ...
  private void checkAndFirePropertyChange(MediaContent oldMc) {
    ...
    boolean eq =
        ((oldMc == null && currentMc == null) || (oldMc != null && oldMc.equals(currentMc)));
    if (!eq) firePropertyChange(PROP_MC, oldMc, currentMc);
  }

  public void setMediaContent(MediaContent c) {

    if (c == null) c = new MediaContent();
    ...
    JToggleButton rb = (JToggleButton) en.nextElement();
    int i = 0;
    for (; i < mc.mediaType; i++) rb = (JToggleButton) en.nextElement();
    rb.setSelected(true);
    ...
    if (mc.absLocation != null) posAbsoluteRadio.setSelected(true);
    else if (mc.free) posWindowRadio.setSelected(true);
    else posCellRadio.setSelected(true);
  ...
  public boolean check(Component parent) {
    ...
    switch (mc.mediaType) {
      ...
      case MediaContent.RUN_CLIC_PACKAGE:
        result =
            StrUtils.nullableString(mc.mediaFileName) != null
    ...
    if (!result) options.getMessages().showAlert(parent, "edit_mc_err_nullContent");
    return result;
  }
  ...
  private void initComponents() {
    ...
    mediaTypePanel.setBorder(
        javax.swing.BorderFactory.createTitledBorder(options.getMsg("edit_mc_type")));
    ...
    lpanel.setBorder(
        javax.swing.BorderFactory.createTitledBorder(options.getMsg("edit_mc_location")));
  ...
  public boolean documentChangePerformed(DocumentEvent ev) {
    ...
    int v = 0;
    for (; v < textFields.length; v++) if (textFields[v].getDocument() == doc) break;
    if (v < textFields.length) return eventPerformed(new EventObject(textFields[v]));
    return false;
  }
  ...
  public boolean eventPerformed(EventObject ev) {
    if (ev != null && ev.getSource() != null) {
      ...
      if (src == levelEditor) mc.level = levelEditor.getValue();
      else if (src == recBufferEditor) mc.recBuffer = recBufferEditor.getValue();
      else if (src == lengthEditor) mc.length = lengthEditor.getValue();
      else if (src == fileTxt) {
      ...
      } else if (src == fromTxt) mc.from = getIntValue(fromTxt, -1);
      else if (src == toTxt) mc.to = getIntValue(toTxt, -1);
      else if (src == stretchChk) mc.stretch = stretchChk.isSelected();
      ...
      else if (src == xTxt && mc.absLocation != null) mc.absLocation.x = getIntValue(xTxt, 0);
      else if (src == yTxt && mc.absLocation != null) mc.absLocation.y = getIntValue(yTxt, 0);
      else if (src == loopChk) mc.loop = loopChk.isSelected();
      else if (src == autostartChk) mc.autoStart = autostartChk.isSelected();
      else if (src == catchMouseChk) mc.catchMouseEvents = catchMouseChk.isSelected();
  ...
  protected void selectBtnPressed() {
    ...
    if (list != null) {
      ...
      int filter =
          mc.mediaType == MediaContent.PLAY_AUDIO
              ? Utils.ALL_SOUNDS_FF
              : mc.mediaType == MediaContent.PLAY_MIDI
                  ? Utils.MIDI_FF
                  : mc.mediaType == MediaContent.PLAY_VIDEO
                      ? Utils.ALL_VIDEO_FF
  ...
  protected void checkEnabled() {
    ...
    if (filePanelItem == 1) {
      String s =
          mc.mediaType == MediaContent.RUN_CLIC_ACTIVITY
              ? "edit_mc_activity"
  ...
  private void syncPointValues() {
    ...
    if (p == null) p = new java.awt.Point();
  ...
  private static Map<Options, MediaContentEditor> panels =
      new HashMap<Options, MediaContentEditor>();

  public static MediaContent getMediaContent(
      MediaContent initialMc, Component parent, Options options, MediaBagEditor mbe) {
    ...
    if (msg.showInputDlg(parent, mce, "edit_mc_title") && mce.check(parent)) result = mce.getMc();
    return result;
  }
