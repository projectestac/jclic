 ...
 * @version 13.09.17
 */
public class EventSoundsEditorPanel extends javax.swing.JPanel
  ...
  public EventSoundsEditorPanel(Options options, MediaBagEditor mbe) {
    ...
    initComponents();
    txFields =
        new JLabel[] {evStart, evClick, evActionError, evActionOk, evFinishedError, evFinishedOk};
    buttons =
        new SoundButton[] {
          evStartBtn,
          evClickBtn,
  ...
  public static boolean nameChanged(EventSounds ev, int type, String oldName, String newName) {
    ...
    if ((type & (Constants.T_AUDIO | Constants.T_MIDI)) != 0) {
      if (ev.parent != null) result |= nameChanged(ev.parent, type, oldName, newName);
  ...
  public void setEventSounds(EventSounds evs) {
    if ((this.evs = evs) == null) this.evs = new EventSounds(null);
    for (int i = 0; i < buttons.length; i++) {
      updateDisplayFor(i);
    }
  }
  ...
  private static Map<Options, EventSoundsEditorPanel> dlgPanels =
      new HashMap<Options, EventSoundsEditorPanel>(1);
