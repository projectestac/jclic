...
public abstract class ReportUtils {
  ...
  // Convert dates of type "yyyy/MM/dd" to Date objects
  ...
  public static Date strToDate(String s, boolean atMidnight) throws Exception {
    ...
    if (s.indexOf('/') > 0 || s.indexOf('-') > 0) {
      ...
      gc = new GregorianCalendar(Integer.parseInt(st.nextToken()), Integer.parseInt(st.nextToken()) - 1,
          Integer.parseInt(st.nextToken()), atMidnight ? 23 : 0, atMidnight ? 59 : 0, atMidnight ? 59 : 0);
    } else if (s.length() >= 8) {
      gc = new GregorianCalendar(Integer.parseInt(s.substring(0, 4)), Integer.parseInt(s.substring(4, 6)) - 1,
          Integer.parseInt(s.substring(6, 8)), atMidnight ? 23 : 0, atMidnight ? 59 : 0, atMidnight ? 59 : 0);
    } else
