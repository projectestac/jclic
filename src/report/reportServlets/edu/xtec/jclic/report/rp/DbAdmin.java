...
public class DbAdmin extends Basic {
  ...
  public static final String ALLOW_GROUP_CREATE = "agc",
      ALLOW_USER_CREATE = "auc",
      SHOW_GROUP_LIST = "shgl",
      SHOW_USER_LIST = "shul",
      USER_TABLES = "ut",
      TIME_LAP = "lap";

  public static final String[] DB_BOOL_KEYS = {
    Reporter.ALLOW_CREATE_GROUPS,
    Reporter.ALLOW_CREATE_USERS,
    Reporter.SHOW_GROUP_LIST,
    Reporter.SHOW_USER_LIST,
    Reporter.USER_TABLES
  };

  public static final String[] DB_BOOL_PARAMS = {
    ALLOW_GROUP_CREATE, ALLOW_USER_CREATE, SHOW_GROUP_LIST, SHOW_USER_LIST, USER_TABLES
  };
  ...
  private void updateSettings() throws Exception {
    ...
    for (int i = 0; i < DB_LITERAL_PARAMS.length; i++) sValues[i] = getParam(DB_LITERAL_PARAMS[i]);
  ...
  private void updateItem(
      String sk, String sv, PreparedStatement updateStmt, PreparedStatement insertStmt)
      throws Exception {
    ...
    if (iRecords != 1) {
      ...
      if (iRecords != 1) throw new Exception(bundle.getString("db_admin_settings_error"));
    }
  }
  ...
  public void body(java.io.PrintWriter out) throws Exception {
    ...
    if (page != MENU) sb.append(" | ").append(linkTo(getUrl(), getTitle(bundle), null));
    ...
    switch (page) {
      ...
      default:
        ...
        sb.append("<p>")
            .append(buttonTo(getUrl(EDIT_SETTINGS), bundle.getString("db_admin_edit_btn"), null));
        sb.append("&nbsp;");
        sb.append(buttonTo(getUrl(EDIT_PWD), bundle.getString("db_admin_pw_edit_btn"), null))
            .append("</p>");
        sb.append("</div>\n");
        sb.append("</form>\n");
    }
  ...
  private void writeEditPwdScript(StringBuilder sb) throws Exception {
    ...
    sb.append("  if (document.forms[0].")
        .append(PW_FIELD)
        .append("2.value==document.forms[0].")
        .append(PW_FIELD)
  ...
  private void writeAdminFields(StringBuilder sb, boolean edit) throws Exception {
    ...
    for (int i = 0; i < DB_BOOL_PARAMS.length; i++) {
      ...
      sb.append(" <input type=\"checkbox\" name=\"")
          .append(DB_BOOL_PARAMS[i])
          .append("\" value=\"")
          .append(ON)
          .append("\"");
      if (!edit) sb.append(" disabled");
      String s = (String) settings.get(DB_BOOL_KEYS[i]);
      if (s != null && s.trim().toLowerCase().equals("true")) sb.append(" checked");
      sb.append("></p>\n");
    }
    for (int i = 0; i < DB_LITERAL_PARAMS.length; i++) {
      sb.append("<p>");
      ...
      if (s != null) sb.append(" value=\"").append(s).append("\"");
      if (!edit) sb.append(" disabled");
